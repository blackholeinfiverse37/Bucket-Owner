...
try:
    from bhiv_bucket import get_ai_firewall, ArtifactClass
    BHIV_BUCKET_AVAILABLE = True
except ImportError:
    BHIV_BUCKET_AVAILABLE = False
    ...
    async def _execute_sequential(self, input_data: Dict) -> Dict:
        ...
        for i, agent_name in enumerate(self.agents):
            ...
            try:
                ...
                # Process agent output through BHIV Bucket firewall if available
                if BHIV_BUCKET_AVAILABLE:
                    try:
                        ...
                        if firewall_result["success"]:
                            ...
                            logger.warning(f"Agent output rejected by BHIV Bucket: {firewall_result.get('reason')}")
                            
                    except Exception as bucket_error:
                        logger.warning(f"BHIV Bucket processing error for {agent_name}: {bucket_error}")
